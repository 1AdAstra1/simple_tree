...
class LearningSwitch13 < Trema::Controller
  ...
  def switch_ready(datapath_id)
    logger.info("Switch #{SWITCH.fetch(datapath_id).inspect} ready")
  ...
  def packet_in(_datapath_id, packet_in)
    logger.info("Packet arrived to #{_datapath_id} in #{packet_in.port} from #{packet_in.source_mac}")
  ...
  def add_forwarding_flow_and_packet_out(packet_in)
    logger.info("Sending packet to #{packet_in.destination_mac}")
    port_no = @fdb.lookup(packet_in.destination_mac)
    logger.info("Destination port: #{port_no}")
